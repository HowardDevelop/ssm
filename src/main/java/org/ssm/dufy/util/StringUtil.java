...
public class StringUtil {
     ...
     * @param String
     *            str 待处理字符串
     * @param char c 长度不够时填充的字符
     * @param int length 返回字符串长度
     * @param boolean isPre 为true，在前补0，或者从最后一位开始截取字符串； 为false，在后补0，或者从第一位开始截取字符串
     * @return 处理后定长字符串
     */
    ...
    public static String shieldMobilePhone(String mobilePhone) {
        return StringUtil.shieldStr(mobilePhone, 3, 7);
    }
    ...
    public static String shieldCertificateNo(String certificateNo) {
        return StringUtil.shieldStr(certificateNo, 10, 14);
    }
    ...
    /**
     * 指定文字列を通貨フォーマットを摘要した文字列を取得します。<br>
     * 例: 12345 ⇒ ￥12,345
     * 
     * @param strNumber
     *            対象文字列
     * @return String 通貨フォーマットを摘要した文字列
     */
    public static String formatCurrency(String strNumber) {
        Locale locale = LocaleUtil.getUserLocale();
        return formatCurrency(strNumber, locale);
    }
